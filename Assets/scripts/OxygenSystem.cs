...
public class OxygenSystem : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        /// I should put this in health, however it's here FOR NOW 
        /// .. i have made an object of HealthSystem to call the deplerationRate from health not oxygen, so we can change rates of change for either bar(s) health/oxygen
        if (oxygenDepleted && healthSystem != null && !isDead)
        {
            float healthDamage = healthSystem.GetdepletionHealthRatePerSecond() * Time.deltaTime;
    ...
    /// <summary>
    ///  Set the Health bar (starting)
    ///  Matf.Clamp keeps the UI bar at the right size.... this needs to be looked into
    /// </summary>
